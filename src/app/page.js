...
const displayStmt=stmtMatched.length>0?(()=>{const seen=new Set();let tS=0,tF=0,tT=0,tV=0;stmtMatched.forEach(o=>{if(!seen.has(o.orderId)){seen.add(o.orderId);tS+=o.saleVND||0;tF+=o.feeVND||0;tT+=o.taxVND||0;tV+=o.vatVND||0}});return{totalSales:tS,totalFees:tF,totalTax:tT,totalVAT:tV,totalAds:activeStmt?.totalAds||0}})():activeStmt;
