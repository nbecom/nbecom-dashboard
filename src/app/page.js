...
function parseStatement(text){const rows=parseCSV(text);if(rows.length<2)return{orderNet:{},totalAds:0,totalFees:0,totalTax:0,totalVAT:0,totalSales:0};const h=rows[0].map(x=>x.toLowerCase().replace(/['"]/g,'').replace(/^\uFEFF/,'').trim());const c={};h.forEach((v,i)=>{if(v==='date')c.d=i;if(v==='type')c.t=i;if(v==='title')c.ti=i;if(v==='info')c.in=i;if(v==='amount')c.a=i;if(v==='fees & taxes')c.f=i;if(v==='net')c.n=i});
...
const data=[];for(let i=1;i<rows.length;i++){const r=rows[i];if(!r||r.length<3)continue;data.push({date:r[c.d]||'',type:r[c.t]||'',title:r[c.ti]||'',info:r[c.in]||'',amount:pVND(r[c.a]),fee:pVND(r[c.f]),net:pVND(r[c.n])})}
const orderIds=[];data.forEach(r=>{if(r.type==='Sale'){const m=r.title.match(/#(\d+)/);if(m)orderIds.push(m[1])}});
const orderNet={};for(const oid of orderIds){const oRows=data.filter(r=>(r.info||'').includes(oid)||(r.title||'').includes(oid));const sale=oRows.filter(r=>r.type==='Sale').reduce((s,r)=>s+r.amount,0);const fees=oRows.filter(r=>r.type==='Fee').reduce((s,r)=>s+r.fee,0);const tax=oRows.filter(r=>r.type==='Tax').reduce((s,r)=>s+r.fee,0);const vat=oRows.filter(r=>r.type==='VAT').reduce((s,r)=>s+r.fee,0);orderNet[oid]={saleVND:sale,feeVND:fees,taxVND:tax,vatVND:vat,netVND:sale+fees+tax+vat}}
const totalAds=data.filter(r=>r.type==='Marketing').reduce((s,r)=>s+r.fee,0);const totalFees=data.filter(r=>r.type==='Fee').reduce((s,r)=>s+r.fee,0);const totalTax=data.filter(r=>r.type==='Tax').reduce((s,r)=>s+r.fee,0);const totalVAT=data.filter(r=>r.type==='VAT').reduce((s,r)=>s+r.fee,0);const totalSales=data.filter(r=>r.type==='Sale').reduce((s,r)=>s+r.amount,0);
return{orderNet,totalAds,totalFees,totalTax,totalVAT,totalSales}}
...
<td style={{...S.td,fontSize:11,color:'var(--text-dim)',maxWidth:180,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}} title={o.address}>{o.address?.substring(0,40)}</td>
...
<div style={{padding:14,background:'var(--bg)',borderRadius:12,marginBottom:12}}><div style={{fontSize:12,fontWeight:600,marginBottom:8}}>📦 Ship đến</div><div style={{fontSize:13,color:'var(--text-muted)',lineHeight:1.6}}><div><b>{sel.buyer}</b></div><div>{sel.address}</div><div>{cF(sel.country)} {sel.country}</div></div></div>
...
function Reports({orders:allOrders,stmtData,images}){
...
case'orders':return<OrdersView orders={ao} userShops={uS} isAdmin={isA} images={images}/>;
case'reports':return<div style={{animation:'fadeSlideUp 0.4s'}}><Reports orders={ao} stmtData={sd} images={images}/></div>;
